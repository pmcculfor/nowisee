...
|--------|---------------|------------------|---------------|------|
| **Database** | Host SQLite for `users` / `sessions`. Account and Bible each open `data/apps/*.db`. Bible verses are keyed by version. Notes are not registered. No `platform.storage`, no `ctx.db`. | Core still has no database. Each app owns its file or remote store. Host identity may later swap to Postgres; that does not rewrite apps. | Notes: open `data/apps/notes.db`, `NotesStore` methods take `ownerId`. Never add a core `BibleRepository` or `NotesRepository`. | **Keep the discipline.** Spec in [`STORAGE.md`](STORAGE.md). |
...
| **First-party apps** | Home, Bible, Account, Notes (unregistered). Registry is compile-time `register()`. Home lists descriptors, not the registry object. App-kit helpers exist. Core has no product names. Help is still a Home node, not an app. The host still names each `start*`. | New app = folder + `AppModule` + one catalog/start row (later: lazy import and per-user enabled flags). Facebook, X, and AI chat are graphs of text/input nodes. Home stays an ordinary app. Help is an app too. | Almost no core work. Domain, URLs, and warm policy stay in the app. Lazy-load large apps so a game or social client does not ship KJV. Collapse the host’s per-app start list — see **Owner follow-ups**. | **Ready now**, aside from the host start list and Help-as-Home-node. Adding apps you write will not get harder once those two land. |
...
Wanted eventually. Neither is a rewrite; both are recorded so they are not treated as accidents of the current host.

| Item | Current | Wanted | Constraint |
|------|---------|--------|------------|
| **Help as an app** | Help is a Home node (`home:help`), a sibling of the catalog labels. | Help is an ordinary `AppModule`. Home lists it the same way it lists Bible and Account — via `listEnabled`, with an `app` edge. | The SPEC lock that help lives **in the tree as nodes, not a modal** still holds. Binding copy moves into that app; Home stops special-casing a help node. |
| **Generic host start** | `createNowiseeHost` names `createHomeApp`, `startBibleApp`, and `startAccountApp`, and `close()` names each again. | A catalog of starters the host walks: for each registered app, start it (and later close it). Adding Notes or Help is a catalog row, not a new host branch. | Each app still owns its own start — opens its own file, seeds, receives granted capabilities. The host must not grow a `switch (appId)` or open app databases itself. This is the “one register line” path in **First-party apps** above; the host is the remaining per-app list. |
