...
- **Bible data:** tests pass a tiny seed. Dev and production import public-domain VPL + commentaries from committed files under `src/apps/bible/data/raw/` via `ensureCatalog` (zips/USFM/SWORD backups stay gitignored; re-fetch with `node scripts/download-bible-sources.mjs`). Do not extend `scripts/prepare-kjv.mjs` / committed `kjv.json` brace-stripping.
