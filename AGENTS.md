...
- **Production:** `npm run build && npm start` — `server/index.ts` serves `dist/` and `/api` together at **https://nowisee.app**. OAuth redirect is `https://nowisee.app/oauth/callback`. Env template: [`.env.production.example`](.env.production.example); systemd unit: [`deploy/nowisee.service`](deploy/nowisee.service). Node does not load `.env` files — systemd `EnvironmentFile=` injects them.
