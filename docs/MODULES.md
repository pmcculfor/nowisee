...
Gestures: swipe right/left → `enter`/`back`; pan up/down → `prev`/`next`; one-second still hold → `recents`. Each of those fires plays a light haptic; a long vertical pan ticks once per `prev`/`next`. Overlay and input body type is 2× default Dynamic Type body.
