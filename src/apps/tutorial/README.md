...
Done (`enter` with `passInputText`, no `action`) goes to a closing node that quotes what they typed, teaches recents (`r`, iPhone double-tap, input Recent Apps), and sends them Home on `enter`. Cancel (`back`) on the input returns to the prompt. Closing-node `back` pops to the input. Welcome `back` is an `app` edge to Home.
