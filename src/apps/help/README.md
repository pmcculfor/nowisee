...
From there, `enter` reaches a recents-intent node, then four sibling list items that wrap on `next` / `prev`. Only the **fourth** item has an `enter` edge, which leads to a typing prompt and then an input node. The first three list items have no `enter` edge.

Done (`enter` with `passInputText`, no `action`) goes to a closing node that quotes what they typed and sends them Home. Cancel (`back`) returns to the prompt. Input chrome includes Cancel, Done, and Recent apps. The closing node’s `enter` and `back` are `app` edges to Home. Welcome `back` is also an `app` edge to Home.
