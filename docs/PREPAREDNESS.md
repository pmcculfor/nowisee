...
| **First-party apps** | Home, Help, Bible, Notes, Gmail, Account. Registry is compile-time `register()`. Home lists descriptors, not the registry object. Help is first in the catalog. App-kit helpers exist. Core has no product names. The host still names each `start*` for apps that own a file. | New app = folder + `AppModule` + one catalog/start row (later: lazy import and per-user enabled flags). Facebook, X, and AI chat are graphs of text/input nodes. Home stays an ordinary app. | Almost no core work. Domain, URLs, and warm policy stay in the app. Lazy-load large apps so a game or social client does not ship KJV. Collapse the host’s per-app start list — see **Owner follow-ups**. | **Ready now**, aside from the host start list. Adding apps you write will not get harder once that lands. |
